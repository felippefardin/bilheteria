...
if ($result->num_rows === 0) {
    die("Você não tem permissão para cancelar este evento.");
}
$stmt->close();

// Atualiza status para cancelado
$update = $mysqli->prepare("UPDATE eventos SET status = 'cancelado' WHERE id = ?");
$update->bind_param("i", $evento_id);
if ($update->execute()) {
    echo "Evento cancelado com sucesso!";
} else {
    echo "Erro ao cancelar evento: " . $update->error;
}
$update->close();
$mysqli->close();
?>